...
'  }\n' +
"}\n\n" +
"制約：ポジティブ比率7:3。ログの具体的内容に必ず言及する。ラッキーアクティビティは明日からすぐできる行動1つ。占い風演出を加える。scoresのカテゴリは活動方針から抽出した実際のカテゴリ名を使うこと。scoreは必ず0〜100の整数で評価すること。";
...
export default function App() {
  ...
  const [mode, setMode] = useState(null); // "api" or "manual"
      ...
      case 0:
        if (!mode) return (
          <div className="flex flex-col items-center text-center space-y-6 pt-8">
            ...
            <h1 className="text-3xl font-bold text-white">7人の星読みたち</h1>
            <p className="text-gray-300 max-w-sm leading-relaxed">活動方針と活動記録を照合して、あなた専属の占い師がフィードバックします。</p>
            <div className="w-full max-w-sm space-y-3 pt-2">
              <button onClick={() => setMode("api")}
                className="w-full bg-gradient-to-r from-purple-700 to-pink-700 hover:from-purple-600 hover:to-pink-600 text-white rounded-2xl p-5 text-left transition-all border-2 border-transparent hover:border-purple-400">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-2xl">⚡</span>
                  <span className="font-bold text-lg">本格版（APIキーを使う）</span>
                </div>
                <p className="text-purple-200 text-sm">AnthropicのAPIキーを入力するだけで全自動分析。コピペ不要。</p>
                <p className="text-purple-300 text-xs mt-1">※ APIキーの取得が必要です（使った分だけ課金）</p>
              </button>
              <button onClick={() => { setMode("manual"); setStep(1); }}
                className="w-full bg-gray-900 hover:bg-gray-800 text-white rounded-2xl p-5 text-left transition-all border-2 border-gray-700 hover:border-gray-500">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-2xl">📋</span>
                  <span className="font-bold text-lg">お試し版（手動で進める）</span>
                </div>
                <p className="text-gray-300 text-sm">プロンプトをコピーしてClaude.aiやChatGPTに貼り付けて使います。</p>
                <p className="text-gray-400 text-xs mt-1">※ 無料で試せます。Claude.aiのアカウントがあればOK。</p>
              </button>
            </div>
          </div>
        );
        return (
          <div className="flex flex-col items-center text-center space-y-5 pt-8">
            <div className="text-6xl">🔮</div>
            <h1 className="text-3xl font-bold text-white">7人の星読みたち</h1>
            <p className="text-gray-300 max-w-sm leading-relaxed text-sm">APIキーを入力して「はじめる」を押してください。</p>
            <div className="w-full max-w-sm space-y-2">
              <p className="text-gray-400 text-xs text-left font-bold">🔑 Anthropic APIキー</p>
              <input
                type="password"
                value={apiKey}
              ...
              />
              <p className="text-gray-600 text-xs">キーは端末内のみで使用され、外部に送信されません。</p>
            </div>
            <div className="flex gap-3 w-full max-w-sm">
              <button onClick={() => setMode(null)} className="flex-1 py-3 rounded-full border border-gray-700 text-gray-400 hover:text-white text-sm">
                ← 戻る
              </button>
              <button onClick={() => setStep(1)} disabled={!apiKey.trim()}
                className={"flex-1 py-3 rounded-full font-bold flex items-center justify-center gap-2 text-sm " + (apiKey.trim() ? "bg-gradient-to-r from-purple-600 to-pink-600 text-white" : "bg-gray-800 text-gray-600 cursor-not-allowed")}>
                はじめる <Sparkles size={16}/>
              </button>
            </div>
          </div>
        );
