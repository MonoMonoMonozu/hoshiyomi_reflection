...
export default function App() {
  ...
  const char = charId === "custom" ? customPreview : CHARACTERS.find(c => c.id === charId);
  ...
  const generateCustomChar = async () => {
    if (!customName.trim()) return;
    if (!apiKey.trim()) {
      // APIキーなしの場合は汎用プロンプトで仮設定
      setCustomPreview({
        id: "custom", name: customName, icon: "✨", color: "blue",
        borderClass: "border-cyan-500", bgClass: "bg-cyan-950", textClass: "text-cyan-300", btnClass: "from-cyan-700 to-blue-700",
        description: "「" + customName + "」風の占い師",
        systemPrompt: "あなたは「" + customName + "」をイメージした占い師です。" + customName + "らしい口調・性格・価値観で振り返りフィードバックを行ってください。その人物らしい言葉遣いと視点で、良い点は褒め、改善点は率直に指摘し、ラッキーアクティビティを提案してください。"
      });
      return;
    }
    ...
    try {
      ...
      let t = text.trim();
      const fenced = t.match(new RegExp("```json?\\s*([\\s\\S]*?)```"));
      ...
      setCustomPreview({
        ...
        color: "blue",
        borderClass: "border-cyan-500", bgClass: "bg-cyan-950", textClass: "text-cyan-300", btnClass: "from-cyan-700 to-blue-700",
        description: parsed.description || "「" + customName + "」風の占い師",
        systemPrompt: parsed.systemPrompt || "あなたは「" + customName + "」をイメージした占い師です。" + customName + "らしい口調で振り返りフィードバックを行ってください。"
      });
    } catch(e) {
      setCustomPreview({
        id: "custom", name: customName, icon: "✨", color: "blue",
        borderClass: "border-cyan-500", bgClass: "bg-cyan-950", textClass: "text-cyan-300", btnClass: "from-cyan-700 to-blue-700",
        description: "「" + customName + "」風の占い師",
        systemPrompt: "あなたは「" + customName + "」をイメージした占い師です。" + customName + "らしい口調・性格・価値観で振り返りフィードバックを行ってください。"
      });
    } finally {
  ...
  const handleParse = () => {
    ...
    try {
      ...
      // json/markdown ブロックがあれば中身だけ取り出す
      const fenced = text.match(new RegExp("```json?\\s*([\\s\\S]*?)```"));
      if (fenced) {
        text = fenced[1].trim();
      } else {
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              {/* カスタムキャラ枠 */}
              <div className={`w-full p-4 rounded-xl border-2 transition-all ${charId==="custom" && customPreview ? "border-cyan-500 bg-cyan-950" : "border-gray-700 bg-gray-900"}`}>
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xl">✨</span>
                  <p className="text-gray-300 font-bold text-sm">カスタム占い師（誰に相談したい？）</p>
                </div>
                <div className="flex gap-2">
                  <input
                    value={customName}
                    onChange={e => { setCustomName(e.target.value); setCustomPreview(null); if(charId==="custom") setCharId(""); }}
                    placeholder="例：矢沢永吉、万菊師匠、本田圭佑..."
                    className="flex-1 bg-black border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-cyan-500 placeholder-gray-600"
                  />
                  <button onClick={generateCustomChar} disabled={!customName.trim() || isGeneratingChar}
                    className={`px-3 py-2 rounded-lg text-sm font-bold transition-all ${customName.trim() && !isGeneratingChar ? "bg-cyan-700 hover:bg-cyan-600 text-white" : "bg-gray-800 text-gray-600 cursor-not-allowed"}`}>
                    {isGeneratingChar ? "⟳" : "生成"}
                  </button>
                </div>
                {customPreview && (
                  <button onClick={() => setCharId("custom")}
                    className={`mt-3 w-full text-left p-3 rounded-xl border-2 transition-all flex items-center gap-3 ${charId==="custom" ? "border-cyan-400 bg-cyan-900" : "border-cyan-700 bg-cyan-950 hover:border-cyan-500"}`}>
                    <span className="text-2xl">{customPreview.icon}</span>
                    <div className="flex-1">
                      <p className="text-white font-bold text-sm">{customPreview.name}</p>
                      <p className="text-cyan-300 text-xs">{customPreview.description}</p>
                    </div>
                    {charId==="custom" && <span className="text-green-400">✓</span>}
                  </button>
                )}
              </div>
            </div>
            <NavButtons onBack={() => setStep(0)} onNext={() => setStep(2)} nextDisabled={!charId || (charId==="custom" && !customPreview)}/>
          </div>
        );
